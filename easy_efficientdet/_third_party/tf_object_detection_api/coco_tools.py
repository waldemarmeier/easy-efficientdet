...
try:
    ...
    raise Exception("For running the evaluation the COCO API must be installed:"
...
COCO_METRIC_NAMES_AND_INDEX = (('Precision/mAP', 0), ('Precision/mAP@.50IOU',
                                                      1), ('Precision/mAP@.75IOU', 2),
                               ('Precision/mAP (small)',
                                3), ('Precision/mAP (medium)',
                                     4), ('Precision/mAP (large)',
                                          5), ('Recall/AR@1', 6), ('Recall/AR@10', 7),
                               ('Recall/AR@100', 8), ('Recall/AR@100 (small)', 9),
                               ('Recall/AR@100 (medium)', 10), ('Recall/AR@100 (large)',
                                                                11))

COCO_KEYPOINT_METRIC_NAMES_AND_INDEX = (('Precision/mAP',
                                         0), ('Precision/mAP@.50IOU',
                                              1), ('Precision/mAP@.75IOU',
                                                   2), ('Precision/mAP (medium)',
                                                        3), ('Precision/mAP (large)',
                                                             4), ('Recall/AR@1', 5),
                                        ('Recall/AR@10', 6), ('Recall/AR@100', 7),
                                        ('Recall/AR@100 (medium)',
                                         8), ('Recall/AR@100 (large)', 9))
    ...
    """
        ...
        if self._iou_type in ['bbox', 'segm']:
            summary_metrics = OrderedDict([
                (name, self.stats[index]) for name, index in COCO_METRIC_NAMES_AND_INDEX
            ])
            ...
            for metric_name, index in COCO_KEYPOINT_METRIC_NAMES_AND_INDEX:
                ...
                summary_metrics['{} ByCategory/{}'.format(metric_name,
                                                          category_name)] = value
  ...
  """
    ...
    for image_id, boxes, classes in zip(image_ids, groundtruth_boxes,
        ...
        groundtruth_export_list.extend(
            ExportSingleImageGroundtruthToCoco(image_id, annotation_id, category_id_set,
                                               boxes, classes))
...
def ExportSingleImageDetectionMasksToCoco(image_id, category_id_set, detection_masks,
  ...
  """
    ...
    for image_id, boxes, scores, classes in zip(image_ids, detection_boxes,
        ...
        detections_export_list.extend(
            ExportSingleImageDetectionBoxesToCoco(image_id, category_id_set, boxes,
                                                  scores, classes))
    if output_path:
        with tf.gfile.GFile(output_path, 'w') as fid:
    ...
    for image_id, masks, scores, classes in zip(image_ids, detection_masks,
        ...
        segment_export_list.extend(
            ExportSingleImageDetectionMasksToCoco(image_id, category_id_set,
                                                  np.squeeze(masks, axis=3), scores,
                                                  classes))
...
def ExportKeypointsToCOCO(image_ids,
  ...
  """
    ...
    for image_id, keypoints, scores, classes in zip(image_ids, detection_keypoints,
                                                    detection_scores,
                                                    detection_classes):
